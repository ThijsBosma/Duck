...
public class BridgeSpawner : MonoBehaviour
{
    ...
    private GameObject _spawnedBridge;
    ...
    public void SpawnBridge()
    {
        ...
        Transform bridgePivot = _spawnedBridge.GetComponentsInChildren<Transform>()[1];

        _plant._grid.GetXZ(transform.parent.parent.parent.position, out int x, out int z, false);
        _spawnedBridge.transform.position = _plant._grid.GetWorldPosition(x, z);
    ...
    {
        Instantiate(_Particles, _particleOffset, Quaternion.identity);
        
        yield return new WaitForSeconds(0.2f);

        _spawnedBridge = Instantiate(_BridgToSpawn);
